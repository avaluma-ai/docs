...
<Steps>
  <Step title="Install the plugin">
    The `livekit-plugins-avaluma` package provides the `AvatarSession` class that connects the agent pipeline to your avatar. The `pyproject.toml` already declares it as a dependency — no extra install step is needed when using Docker:
