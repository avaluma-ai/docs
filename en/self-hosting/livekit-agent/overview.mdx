...
Both agents rely on `AvatarSession` from the `livekit-plugins-avaluma` package. You instantiate it with your credentials, call `avatar.start()`, and it handles joining the room as a separate avatar participant and forwarding audio frames to the avatar server for rendering.
