...
- Receives audio from the LiveKit Agent via the `livekit-plugins-avaluma` plugin
...
- **`AvatarSession`** — the `livekit-plugins-avaluma` plugin forwards the TTS audio to the Avatar Server, which animates the avatar and streams the video back into the room
