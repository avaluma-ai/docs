...
<ParamField query="webaudio" type="string" default="on">
  Diagnostic switch for audio output. By default the agent's audio is routed through the browser's Web Audio API (`on`). With `off` the plain audio element plays instead. Intended for troubleshooting only, e.g. to narrow down audio or lip-sync issues on mobile devices. **Note:** in Firefox the agent may stay silent inside embedded pages with `off` — remove the switch after testing.
</ParamField>
...
Like the kiosk, the embedded agent optimizes playback automatically: on iPad/iPhone audio plays at full volume, on Android it routes to the loud system speaker. Two icons sit in the top right of the iframe: a **speaker icon** for the agent's volume (independent of the device volume, and remembered between conversations) and a **gear icon** for audio processing and output device. See the kiosk [Sound & volume](/en/agents/kiosk#sound--volume) section for details, including what happens [if the microphone fails](/en/agents/kiosk#if-the-microphone-fails) and the known limitation with Bluetooth headsets in Firefox on Android.
