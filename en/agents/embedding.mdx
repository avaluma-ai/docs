...
| `avaluma:setMicrophone` | `{ enabled }` | Turns the visitor's microphone on (`true`) or off (`false`). Turning it off fully releases the microphone — the device's microphone indicator turns off, and re-enabling takes a brief moment. Use this to build your own push-to-talk button on your page, for example. |
