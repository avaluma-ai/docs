...
Like the kiosk, the embedded agent optimizes playback automatically: on iPad/iPhone audio plays at full volume, on Android it routes to the loud system speaker. Two icons sit in the top right of the iframe: a **speaker icon** for the agent's volume (independent of the device volume, and remembered between conversations) and a **gear icon** for audio processing and output device. See the kiosk [Sound & volume](/en/agents/kiosk#sound--volume) section for details.
