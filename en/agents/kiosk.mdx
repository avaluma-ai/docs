...
<ParamField query="mic" type="string" default="open">
  Microphone mode. `open` keeps the microphone on for the whole conversation; `ptt` enables a push-to-talk button the visitor holds to speak. In PTT mode the microphone is fully released between presses — the device's microphone indicator turns off, and on iPads/iPhones the agent plays at full media volume instead of the quieter "call" volume.
</ParamField>
