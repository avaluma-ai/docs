...
<Steps>
  <Step title="Plugin installieren">
    Das Paket `livekit-plugins-avaluma` stellt die `AvatarSession`-Klasse bereit, die die Agent-Pipeline mit deinem Avatar verbindet. Die `pyproject.toml` deklariert es bereits als Abhängigkeit — bei Verwendung von Docker ist kein zusätzlicher Installationsschritt nötig:
