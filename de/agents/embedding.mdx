...
| `avaluma:setMicrophone` | `{ enabled }` | Schaltet das Mikrofon des Besuchers an (`true`) oder aus (`false`). Beim Ausschalten wird das Mikrofon vollständig freigegeben — der Mikrofon-Indikator des Geräts erlischt, das Wiedereinschalten braucht dafür einen kurzen Moment. Damit baust du z. B. einen eigenen Push-to-Talk-Button auf deiner Seite auf. |
